...
from core.hsi_load_data import normalize
import tifffile
...
def spectral_standardization(data, wavenum_1=2700, wavenum_2=3100, num_samp=61, background=None, br_shift=0):
...
def main():
    # Setup parameters
    base_directory = "/Volumes/ADATA SE880/ADATA Backup/HuBMAP/HuBMAP CosMx/CosMx HSI/HSI"
    num_samp = 61
    wn_1 = 2700
    wn_2 = 3100
    ch_start = int((2800 - wn_1) / (wn_2 - wn_1) * num_samp)
    shift = 20  # Shift molecule spectra by 15 indices to the right
    ...
    for img_path in tqdm(dataset.img_list, desc="Processing images"):
        ...
        img_norm = spectral_standardization(img_spectra, wavenum_1=wn_1, wavenum_2=wn_2, num_samp=num_samp, background=background, br_shift=0)
        ...
        norm_hsi_output_dir = os.path.join(os.path.dirname(os.path.dirname(img_path)), 'Normalized_HSI')
        os.makedirs(norm_hsi_output_dir, exist_ok=True)

        tifffile.imwrite(
            os.path.join(norm_hsi_output_dir, f"normalized-{sample}"), 
