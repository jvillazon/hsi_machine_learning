...
class HSI_Loader:
    def __init__(self, background_df, excluded_mol=None, wavenumber_start=2700, wavenumber_end=3100, num_samples=61, shift=0):
        """
        Initialize HSI Data Loader
        
        Args:
            background_df: pandas DataFrame with background spectrum data
            excluded_mol: list of molecules to exclude (default None)
        ...
        self.excluded_mol = excluded_mol

    def save_molecule_dataset(self, molecule_file, output_path=None):
        """
        ...
        # Remove excluded molecules if specified
        if self.excluded_mol is not None:
            keep = [i for i in keep if temp_names[i] not in self.excluded_mol]
    ...
    def save_srs_params(self, data_dir, output_path=None):
        ...
        # Background shift
        self.background = create_background(self.wavenumber_start, self.wavenumber_end, self.num_samples, self.background_df, br_shift=0)
...
if __name__ == '__main__':
    ...
    CONFIGS = [
        {
            'molecule_file': 'unprocessed_data/averaged_spectra.xlsx',
            'experimental_dir': 'training_data/61-2700-3100/',
            'wavenumber_start': 2700,
            'wavenumber_end': 3100,
            'num_samples': 61,
            'shift': 2,
            'molecule_output': 'averaged_spectra_wn-61',
            'params_output': 'srs_params_61'
        },
        # Add more molecule datasets here as needed
        # {
        #     'molecule_file': 'unprocessed_data/lipid_subtype_VV.xlsx',
        #     'experimental_dir': 'training_data/61-2700-3100/',
        #     'wavenumber_start': 2700,
        #     'wavenumber_end': 3100,
        #     'num_samples': 61,
        #     'shift': 2,
        #     'molecule_output': 'lipid_subtype_',
        #     'params_output': 'srs_params_VV_61'
