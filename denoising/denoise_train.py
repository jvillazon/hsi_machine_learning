...
def visualize_reconstructions(models, model_names, test_loader, device, save_dir, num_samples=4):
    ...
    for i in range(num_samples):
        ...
        ax.set_title(f'Residuals (Reconstructed - Target)')
        ax.set_xlabel('Wavenumber Index')
...
def main():
    ...
    SAVE_DIR = Path('denoising_results_loss_comparison')
    ...
    dataset = HSI_Denoising_Dataset(
        molecule_dataset_path='molecule_dataset/lipid_subtype_wn-61',
    ...
    loss_configs = [
        # Shape + MSE only (50% shape correlation, 50% reconstruction accuracy)
        {'name': 'Correlation+MSE', 'w_shape': 0.5, 'w_grad': 0.0, 'w_curv': 0.0, 'w_mse': 0.5},
    ]
