...
def main():
    ...
    mask_dir = r"/Users/jorgevillazon/Documents/files/integrated_pipeline/HSI_data/Mask"       # Directory containing mask files
    image_dir = r"/Users/jorgevillazon/Documents/files/integrated_pipeline/HSI_data/data"     # Directory containing images to be masked
    # output_dir = os.path.join(os.path.dirname(image_dir), "output_masks") 
    ...
    threshold_dir = os.path.join(os.path.dirname(image_dir), "threshold_masks")
    os.makedirs(threshold_dir, exist_ok=True)

    # Threshold maks images in image_dir
    for image_path in tqdm(glob.glob(os.path.join(image_dir, "*.tif")), desc="Processing images"):
        image_filename = os.path.basename(image_path)
        image_name = os.path.splitext(image_filename)[0]  # Get name without extension
        ...
        # Apply otsu thresholding on mip
        mip = np.max(image_array, axis=0)
        # threshold = threshold_otsu(mip)

        # Apply local thresholding
        threshold = threshold_multiotsu(mip, classes=3)
        mask = mip >= threshold[0]
        masked_array = image_array * mask
        output_path = os.path.join(threshold_dir, f"{image_name}.tif")
        tifffile.imwrite(output_path, masked_array)
