...
for sample in tqdm(sample_list):
    ...
    # X_norm = helper_scripts.normalize(X_norm, max=np.mean(image_norm)+np.std(image_norm))
    ...
    num_estimators = 500
    rfc = load(
        'rf_classifiers/rfc-' + smooth + '_' + str(wavenum_1) + '_' + str(wavenum_2) + '_' + str(num_samp) + '_' + str(
            num_estimators) + '.joblib')
